#!/usr/bin/env bash
# Adaptador Codex do onboarding canônico; apresenta somente documentos e skills selecionáveis.
set -euo pipefail
if [ ! -t 0 ]; then cat >/dev/null || true; fi
raiz="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd -P)"
documentos="$(find "$raiz/docs" -type f -name '*.md' ! -path "$raiz/docs/agentes/*" ! -path "$raiz/docs/skills/*" ! -path "$raiz/docs/roles/*" -printf '%P\n' | LC_ALL=C sort | sed 's#^#- docs/#')"
skills="$(find "$raiz/docs/skills" -maxdepth 1 -type f -name '*.md' -printf '%f\n' | LC_ALL=C sort | sed 's#\.md$##' | sed 's#^#- #')"
